const ADMIN_CACHE="yummypro-admin-v2339-pages-safe";
const ADMIN_SCOPE=self.registration.scope;
const adminAsset=name=>new URL(name,ADMIN_SCOPE).toString();
const ADMIN_OFFLINE=adminAsset("offline.html");
const ADMIN_CORE=["offline.html","manifest.webmanifest","pwa-icon.svg","icon-192.png","icon-512.png","apple-touch-icon.png"].map(adminAsset);
...
self.addEventListener("fetch",event=>{
 ...
 if(ADMIN_CORE.includes(url.href)){
...
self.addEventListener("push",event=>{
 ...
 event.waitUntil((async()=>{
  ...
  const options={body:data.body||"Tienes una actualización.",icon:adminAsset("icon-192.png"),badge:adminAsset("icon-192.png"),tag:data.tag||"yummypro-admin-notification",renotify:true,silent:!!data.silent,requireInteraction:!!data.requireInteraction,data:{url:data.url||"./"}};
...
self.addEventListener("notificationclick",event=>{
 ...
 const target=new URL(event.notification.data?.url||"./",ADMIN_SCOPE).href;
 event.waitUntil(clients.matchAll({type:"window",includeUncontrolled:true}).then(list=>{
  for(const client of list){if(client.url.startsWith(ADMIN_SCOPE)){client.navigate(target);return client.focus()}}
