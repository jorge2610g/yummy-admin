...
for(const marker of ['manifest.webmanifest','adminPwaInstall','installAdminPwa','adminPwaNetwork','adminPwaUpdate','admin-pwa-script-v2320','Versión v2.3.73'])if(!admin.includes(marker))throw new Error('index.html: falta PWA Admin '+marker);
...
for(const marker of ["originalOpenRestaurantPanel","return originalOpenRestaurantPanel(id,section)","mismo ticket temporal de un solo uso"])if(!streamingPreviewHotfix.includes(marker))throw new Error('admin-streaming-access.js: falta delegación segura de vista administrativa '+marker);
for(const forbidden of ["#admin_access=","#admin_refresh=","session.refresh_token","refresh_token:session.refresh_token"])if(streamingPreviewHotfix.includes(forbidden))throw new Error('admin-streaming-access.js: conserva transferencia insegura de sesión '+forbidden);
...
for(const marker of ['id="viewMenuLink" class="hidden"','onclick="openCurrentBusinessMenu(event);return false"','YUMMY_CLIENT_MENU_BASE','https://menu.yummypro.online/','https://jorge2610g.github.io/yummy-cliente-pruebas/','adminClientMenuUrl','openCurrentBusinessMenu','dashboardRestaurantScope&&dashboardRestaurantScope!=="all"','window.adminBusinessRows','limit(2)','menuLink.dataset.restaurantId=String(currentRestaurant)','menuLink.href=adminClientMenuUrl(menuSlug)','menuLink.classList.remove("hidden")'])if(!admin.includes(marker))throw new Error('index.html: falta enrutamiento seguro del acceso Ver menú '+marker);
