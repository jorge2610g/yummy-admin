...
for(const marker of ['manifest.webmanifest','adminPwaInstall','installAdminPwa','adminPwaNetwork','adminPwaUpdate','admin-pwa-script-v2320','Versión v2.3.72'])if(!admin.includes(marker))throw new Error('index.html: falta PWA Admin '+marker);
...
for(const marker of ['id="viewMenuLink" class="hidden"','onclick="openCurrentBusinessMenu(event);return false"','YUMMY_CLIENT_MENU_BASE','https://menu.yummypro.online/','https://jorge2610g.github.io/yummy-cliente-pruebas/','adminClientMenuUrl','openCurrentBusinessMenu','menuLink.dataset.restaurantId=String(currentRestaurant)','menuLink.href=adminClientMenuUrl(menuSlug)','menuLink.classList.remove("hidden")'])if(!admin.includes(marker))throw new Error('index.html: falta enrutamiento seguro del acceso Ver menú '+marker);
